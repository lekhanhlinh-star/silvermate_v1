...
export default function LineCallback() {
  ...
  useEffect(() => {
    ...
    console.log("LINE OAuth callback params:", { code: code?.substring(0, 20) + "...", state, errorParam });

    if (errorParam) {
      setError("LINE sign-in was cancelled or failed.");
      return;
    }
    ...
    (async () => {
      try {
        console.log("Calling lineCallback with:", {
          code: code.substring(0, 20) + "...",
          state: state?.substring(0, 50) + "...",
        });
        const res = await auth.lineCallback(code, state || undefined);
        if (res.access_token) {
          setToken(res.access_token);
          let user = await users.me();

          const pendingType = localStorage.getItem("pending_line_user_type");
          if (pendingType && (pendingType === "PARENT" || pendingType === "CHILD")) {
            try {
              user = await users.updateMe({ user_type: pendingType });
            } catch (updateErr) {
              console.error("Failed to update user type during LINE signup", updateErr);
            }
            localStorage.removeItem("pending_line_user_type");
          }
        ...
        setError(err.message || "Failed to complete LINE sign-in.");
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-muted-foreground font-body text-lg">Completing LINE sign-in...</p>
      </div>
    </div>
  );
}
